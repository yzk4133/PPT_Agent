...
class FetchUrlInput(BaseModel):
    ...
    extract_type: Literal["full", "main_content", "article", "text_only"] = Field(
        default="main_content", description="Type of content extraction"
    )
...
@monitor_tool
async def fetch_url(
    url: str, timeout: int = 10, extract_type: str = "main_content", use_cache: bool = True
) -> dict:
    """
...
async def _extract_content(html: str, extract_type: str, url: str) -> dict:
    ...
    result = {"url": url, "title": "", "fetched_at": datetime.now().isoformat()}
