...
try:
    ...
    from cognition.memory.core.core.memory_layer_base import MemoryScope
    from cognition.memory.core.services.shared_workspace_service import (
        SharedWorkspaceService,
    )
    from cognition.memory.core.services.user_preference_service import (
        UserPreferenceService,
    )
    from cognition.memory.core.services.agent_decision_service import (
...
class AgentMemoryMixin:
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.memory_enabled:
            try:
                ...
                logger.info(
                    f"[{self.__class__.__name__}] 记忆服务初始化成功"
                )
            except Exception as e:
                logger.warning(
                    f"[{self.__class__.__name__}] 记忆服务初始化失败: {e}"
                )
                self.memory_enabled = False
        else:
            if not _MEMORY_AVAILABLE:
                logger.debug(
                    f"[{self.__class__.__name__}] 记忆系统不可用，无记忆模式"
                )
            else:
                logger.debug(
    ...
    async def get_shared_data(
        ...
        try:
            ...
            if data:
                logger.debug(f"[{self.agent_name}] 获取共享数据: {data_type}:{data_key}")
    ...
    def _infer_scope_from_key(self, key: str) -> str:
        ...
        if any(keyword in key_lower for keyword in ["state", "status", "progress"]):
            return "TASK"
        elif any(keyword in key_lower for keyword in ["research", "cache", "framework"]):
            return "USER"
