...
try:
    ...
    logging.warning("Progressive loading framework not available")
...
if __name__ == "__main__":
    ...
    async def demo():
        ...
        if PROGRESSIVE_LOADING_AVAILABLE:
            ...
            # 创建优化器
            optimizer = ContextWindowOptimizer(skill_registry=registry)

            # 构建上下文
            result = await optimizer.build_optimized_context(
                agent_name="ResearchAgent",
                task_description="I need to research AI trends in 2024",
                user_id="user_123",
                session_id="sess_456",
                required_skills=["research-guide"],
                token_budget=5000,
            )
