...
class ActiveScopeTracker:
    ...
    async def cleanup_inactive(self):
        ...
        async with self._lock:
            now = datetime.now()
            expired_keys = [
                key for key, last_active in self._active_scopes.items()
                if now - last_active > timedelta(seconds=self.ttl_seconds)
            ]
    ...
    def should_promote_l1_to_l2(
        ...
        if reasons:
            ...
            if PromotionReason.HIGH_IMPORTANCE_SCORE in reasons:
                return True, PromotionReason.HIGH_IMPORTANCE_SCORE, "; ".join(explanations)
            elif PromotionReason.HIGH_ACCESS_FREQUENCY in reasons:
                return True, PromotionReason.HIGH_ACCESS_FREQUENCY, "; ".join(explanations)
            else:
                return True, reasons[0], "; ".join(explanations)

        return False, None, f"No promotion rule met: access_count={metadata.access_count}, importance={metadata.importance:.2f}"
    ...
    def should_promote_l2_to_l3(
        ...
        if reasons:
            ...
            if PromotionReason.CROSS_SESSION_USAGE in reasons:
                return True, PromotionReason.CROSS_SESSION_USAGE, "; ".join(explanations)
            else:
                return True, reasons[0], "; ".join(explanations)

        return False, None, f"No promotion rule met: cross_session={cross_session_count}, session_ids={len(metadata.session_ids)}, access_count={metadata.access_count}, importance={metadata.importance:.2f}"
...
class DataMigrator:
    ...
    def __init__(self):
        """初始化迁移器"""
        self._migration_stats = defaultdict(lambda: {
            "total": 0,
            "success": 0,
            "failed": 0,
        })
...
class PromotionEngine:
    ...
    def __init__(
        ...
        # 初始化组件
        self.scope_tracker = ActiveScopeTracker(ttl_seconds=self.config.ACTIVE_SCOPE_TTL)
    ...
    async def promote_l1_to_l2(
        ...
        for scope, scope_id in active_scopes:
            ...
            for key in keys:
                ...
                # 评估提升规则
                should_promote, reason, explanation = self.rule_engine.should_promote_l1_to_l2(metadata)
    ...
    async def promote_l2_to_l3(
        ...
        for scope, scope_id in active_scopes:
            ...
            for key in keys:
                ...
                # 评估提升规则
                should_promote, reason, explanation = self.rule_engine.should_promote_l2_to_l3(
