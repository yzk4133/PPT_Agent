...
class ADKHostManager(ApplicationManager):
    """基于内存管理的实现，包含模拟的 agent 动作

    这实现了 ApplicationManager 的接口，用于插入 AgentServer。
    这作为 Mesop 应用使用的服务契约，用于向 agent 发送消息并为前端提供信息。
    """
    ...
    def __init__(
        ...
        # 用于管理"丢失"消息 ID 的映射，直到引入协议级别的 ID
        self._next_id: dict[
            str, str
        ] = {}  # dict[str, str]: 前一条消息到下一条消息
    ...
    async def adk_content_to_message(
        ...
        for part in content.parts:
            ...
            # 这些不由 A2A 消息结构管理，这些是 ADK 的内部细节，
            # 我们将简单地将这些展平为 json 表示形式。
    ...
    def process_message_threadsafe(
        ...
        """使用给定的事件循环从线程安全地运行 process_message"""
        future = asyncio.run_coroutine_threadsafe(
            self.process_message(message), loop
        )
        return (
            future  # 如果需要，可以调用 future.result() 获取结果
        )
