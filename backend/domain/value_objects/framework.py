...
class PageDefinition(ValueObject, Serializable):
    """
    页面定义
...
@dataclass(frozen=True)
class PPTFramework(ValueObject, Serializable):
    """
    PPT框架模型
    ...
    def __post_init__(self):
        ...
        if self.total_page < 1:
            raise ValidationError("Framework must have at least 1 page")

        # 验证页面数量匹配
        if self.pages and len(self.pages) != self.total_page:
            raise ValidationError(
                f"Page count mismatch: expected {self.total_page}, got {len(self.pages)}"
            )

    @property
    def research_count(self) -> int:
        """需要研究的页面数量"""
        return len(self.research_page_indices)

    @property
    def chart_count(self) -> int:
        """需要图表的页面数量"""
        return len(self.chart_page_indices)

    @property
    def image_count(self) -> int:
        """需要配图的页面数量"""
        return len(self.image_page_indices)
