...
export const convertToColumns = (
  editor: PlateEditor,
  path1: Path,
  path2: Path,
) => {
  withoutNormalizing(editor, () => {
    const entry1 = getNodeEntry<TElement>(editor, path1);
    const entry2 = getNodeEntry<TElement>(editor, path2);
    ...
    // Check if either node is already in a column
    const isColumn1 = isType(editor, node1, ColumnPlugin.key);
    const isColumn2 = isType(editor, node2, ColumnPlugin.key);

    if (isColumn1 || isColumn2) return;

    // Create a column group with both elements
